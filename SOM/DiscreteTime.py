...
def open_output_files(K,B,omega,mu,alphasqrd,n,k,rep):
	...
	sim_id = 'K%d_B%d_w%d_u%.4f_alphasqrd%.2f_n%d_k%.2f_rep%d' %(K,B,omega,mu,alphasqrd,n,k,rep)
	data_dir = 'data'
...
keep_n = 10 #number of gens of data to keep in case popn goes extinct
...
def main():
	...
	for i in range(len(ks)):
		...
		while rep < nReps:
			...
			stored_data = [] #list to keep data from last keep_n generations to use in case popn goes extinct
			...
			while gen < maxgens + 1:
				...
				w = np.exp(-(dist**2)/(2*omega**2)) #probability of survival
				...
				# store some data in case of extinction
				stored_data.append([len(genos),genotypes,phenos,gen,numparents]) #add this gens data to stored data
				stored_data = stored_data[-keep_n:] #trim to last keep_n gens				

				#end simulation if extinct (need at least two individuals for mating)        
				if len(genos) < 2: 
					#record first
					for i in range(keep_n):
						write_data_to_output(fileHandles, stored_data[i]) #write data from last keep_n gens 
				...
				if (gen % outputFreq) == 0:
					write_data_to_output(fileHandles, stored_data[-1])
					print("k %.3f     rep %d    gen %d    N %d" %(k, rep, gen, len(genos)))  
...
if plotting:
	...
	rep = 0
	sim_id = 'K%d_B%d_w%r_u%.4f_alphasqrd%.2f_n%d_k%.2f_rep%d' %(K,B,omega,mu,alphasqrd,n,k,rep)
	data_dir = 'data'
	...
	plt.show()
	fig1.savefig('data/plot_%s.png' %(sim_id))
